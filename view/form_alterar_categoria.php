...
<?php
        ...
        $c = new Categoria();
        $c = $cDAO->carregarPorId($idCategoria);
?>
<form action="../controller/alterar_categoria.php" method="POST">
    <div class="mb-3">
      <label for="" class="form-label">Nome do Cargo</label>
      <input type="hidden" name="id_categoria" value="<?=$c->getIdCategoria() ?>"/>
      <input type="text" class="form-control" name="nome" id="nome" value="<?=$c->getNome() ?>" aria-describedby="" required>
    </div>
