...
<?php
        ...
        $uDAO = new UsuarioDAO();
        $u = new Usuario();
        $u = $uDAO->carregarPorId($idUsuario);
?>
<form action="../controller/alterar_usuario.php" method="POST">
    <div class="mb-3">
    <input type="hidden" name="id_usuario" value="<?=$u->getIdUsuario() ?>"/>
      <label for="" class="form-label">Nome do Usuario</label>
      <input type="text" class="form-control" name="nome" id="nome" value="<?=$u->getNome() ?>" aria-describedby="" required></br>
      <label for="" class="form-label">E-mail</label>
      <input type="text" class="form-control" name="email" id="email" value="<?=$u->getEmail() ?>" aria-describedby="" required></br>
      <label for="" class="form-label">CPF</label>
      <input type="text" class="form-control" name="cpf" id="cpf" value="<?=$u->getCpf() ?>" aria-describedby="" required></br>
    ...
    <?php
            ...
            foreach ($lista as $c) {
            ?>
            <option value="<?=$c->getCodCargo()?>"><?= $c->getNome()?></option>
            <?php
            }
            ?>
</select>
